...
export default function Dashboard() {
  ...
  const fetchTickets = async () => {
    try {
      ...
      const formatted = data.map((t: any): Ticket => ({
        ...
        code: t.code || `TK-${Math.floor(1000 + Math.random() * 9000)}`
      }));
  ...
  const getTabCount = (status: string) => status === 'all' ? tickets.length : tickets.filter(t => t.status === status).length;
  ...
  return (
    <div className="min-h-screen bg-[#F9F9F9]">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
            <div className="flex items-center space-x-4 lg:space-x-8">
              ...
              <nav className="flex space-x-4 lg:space-x-6">
                ...
                <Link href="/dashboardC/feedback" className="text-gray-600 hover:text-[#F08080] transition-colors">
                  Feedback
                </Link>
                <Link href="/dashboardC/settings" className="text-gray-600 hover:text-[#F08080] transition-colors">
                  Paramètres
                </Link>
                <Link href="/dashboardC/account" className="text-gray-600 hover:text-[#F08080] transition-colors">
                  Mon compte
                </Link>
              </nav>
            </div>
      ...
      {/* Conteneur principal avec marges appropriées */}
        ...
        <div className="bg-white rounded-lg shadow-sm">
          <div className="p-4 lg:p-6 border-b border-gray-200">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
              ...
              <Link href="/dashboardC/new-ticket" className="bg-[#F08080] text-white px-6 py-2 rounded-lg hover:bg-[#F4978E] transition-colors flex items-center justify-center space-x-2 whitespace-nowrap">
            ...
            <nav className="flex space-x-4 lg:space-x-8 px-4 lg:px-6 overflow-x-auto">
              {['all', 'pending', 'resolved', 'transferred'].map(tab => (
                <button
                  ...
                  className={`py-4 px-1 border-b-2 font-medium text-sm whitespace-nowrap ${activeTab === tab ? 'border-[#F08080] text-[#F08080]' : 'border-transparent text-gray-500 hover:text-gray-700'
                    }`}
                >
          ...
          <div className="p-4 lg:p-6">
            {filtered.length > 0 ? (
              ...
              <div className="text-center py-12">
                ...
                <Link href="/dashboardC/new-ticket" className="bg-[#F08080] text-white px-6 py-2 rounded-lg hover:bg-[#F4978E] transition-colors inline-flex items-center space-x-2 whitespace-nowrap">
                  <i className="ri-add-line"></i>
